...
public class RaftCore implements Closeable {
    ...
    public void signalPublish(String key, Record value) throws Exception {
        ...
        if (!isLeader()) {
            ...
            // 如果当前节点不是leader节点，那么将写请求（注册实例）转发给leader节点处理。
            // 注：Raft协议中只有leader节点才能处理写请求
        ...
        private void sendBeat() throws IOException, InterruptedException {
            ...
            for (final String server : peers.allServersWithoutMySelf()) {
                try {
                    ...
                    // /raft/beat：com.alibaba.nacos.naming.controllers.RaftController.beat
    ...
    public RaftPeer receivedBeat(JsonNode beat) throws Exception {
        ...
        // 当前节点纪元大于发心跳节点的纪元的时候，不接受心跳请求。
        // 此时可能发生了网络分区，发心跳的可能是少数节点分区的leader节点，这时候不能接受心跳，需要对方认可我为新的leader节点。
        ...
        // 如果收到心跳了，并且自己不是从节点，那么将自己设置为从节点。因为发心跳的节点一定是主节点。前面已经判断过纪元（term）了。
        ...
        if (!switchDomain.isSendBeatOnly()) {
            ...
            for (Object object : beatDatums) {
                ...
                // 本地的数据的value=0
                // 主节点心跳中携带的数据value=1
                // 等到主节点数据全部处理完成之后，这个map中所有值为0的就是本地有主节点没有的数据，也就是这些数据在主节点已经被删除了，本地也需要同步移除。
                // 这里的魔法值很差劲！
                ...
                try {
                    ...
                    // 最多50条一个批次，向主节点批量请求最新的数据，并以此更新到本地。
